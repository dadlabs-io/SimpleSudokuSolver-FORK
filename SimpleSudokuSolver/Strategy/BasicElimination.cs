    ...
    public class BasicElimination : ISudokuSolverStrategy
    {
        ...
        public SingleStepSolution SolveSingleStep(SudokuPuzzle sudokuPuzzle)
        {
            ...
            foreach (var column in sudokuPuzzle.Columns)
            {
                ...
                // DEBUG: Trace column 8 (index 7) to find the 4 issue
                if (Model.SudokuPuzzle.EnableVerboseFileLogging && column.ColumnIndex == 7)
                {
                    var withValue = cells.Where(x => x.HasValue).ToList();
                    var noValue = cells.Where(x => !x.HasValue).ToList();
                    var logLines = new List<string>();
                    logLines.Add($"[BasicElim] Column 8: {withValue.Count} placed, {noValue.Count} empty");
                    foreach (var c in withValue)
                    {
                        logLines.Add($"  Placed: R{c.RowIndex + 1}C8 = {c.Value}");
                    }
                    System.IO.File.AppendAllLines(@"c:\github.com\sudoku-app\memory-bank\short-term\logs\basicelim-debug.log", logLines);
                }
